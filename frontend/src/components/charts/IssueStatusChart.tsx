...
export const IssueStatusChart = ({ open, closed }: { open: number; closed: number }) => {
  if (open === 0 && closed === 0) {
    return (
      <div className="flex h-64 flex-col justify-center rounded-xl border border-slate-800 bg-slate-900 p-5">
        <h3 className="text-lg font-semibold text-white">Issue Status</h3>
        <div className="flex flex-1 items-center justify-center">
          <p className="text-sm text-slate-400">No issue activity available</p>
        </div>
      </div>
    );
  }
  ...
  const data = [
    { name: "Open", value: open, color: "#eab308" },
